...
export default class Income extends Vue {
  ...
  private onConfirm(value: any, index: number) {
    ...
    const target = (incomeInfo as any).filter((item: ProblemList) => item.problemFlag === 'income')[0]
    ...
    return (
      <van-row class="income">
        ...
        <van-col span="24" class="income-input">
          <van-cell-group>
            <van-cell title="家庭年收入" value={target.answer} onClick={this.handleIncome} is-link />
          </van-cell-group>
        </van-col>
